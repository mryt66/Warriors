import sqlite3
from warrior_creator import Warrior_creator, Warriors


def readSqliteTable():
    try:
        sqliteConnection = sqlite3.connect('data_warriors.db', timeout=20)
        cursor = sqliteConnection.cursor()
        print("Connected to SQLite")
        sqlite_select_query = """SELECT * from Fighters"""
...
def add_tobase(filepath, my_objects):  # dodaje 1 element
    ...
    for name in my_objects:
        file.write(str(my_objects[name].name) + " " + str(my_objects[name].ad) + " " + str(
            my_objects[name].health) + " " + str(my_objects[name].agility) + "\n")
    file.close()
...
def read_frombase(filepath, my_objects):
    ...
    for i in file:
        ...
        if new_object[0] not in my_objects:
            my_objects[new_object[0]] = Warriors(new_object[0], int(new_object[1]), int(new_object[2]),
                                                 int(new_object[3]))
    print(my_objects)
...
def available_warriors(my_objects):
    if len(my_objects) != 0:
        print(f"Your available warriors: \n"
              f"{'Name':15s}|{' AD ':3s}|{'Health':5s}|{'Agility':2s}")
        for j in my_objects:
            print(
                f"{my_objects[j].name:15s}|{my_objects[j].ad:3} |{my_objects[j].health:4}  |  {my_objects[j].agility:2}")
    else:
...
def fight_pattern(my_objects):
    if len(my_objects) >= 2:  # implementing data from my_objects dictionary
        ...
        while end == True:
            [my_objects[id_warrior1].health, my_objects[id_warrior2].health, end] = fight(id_warrior1, id_warrior2,
                                                                                          DMG1, DMG2, (my_objects[
                                                                                                           id_warrior1].health),
                                                                                          (my_objects[
                                                                                               id_warrior2].health))
    ...
    if ans1 == 1 or ans1 == 2 or ans1 == 3:
        if ans1 == 1:
            print(
                ...
                "Fight process works in the way that fighter gets minus health points"
                "which are equal to total damage of different fighter\n"
                "Total damage=Attack damage multipled by (1+(agility/100))\n"
                "The fight is over when one of the fighters is dead "
                "it means that his health is equal or lower than zero\n")
            menu()
        elif ans1 == 2:
            game_menu()
        elif ans1 == 3:
